...
/*   Updated: 2025/10/08 16:07:20 by ldevoude         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool	element_found(int fd_sd, t_parsing *parsing, char first_letter,
		int element_type)
{
	if (read(fd_sd, parsing->buff, 1) == -1)
		return (error_handler(parsing, INV_READ, "parsing_collect_elements.c:23 ",
				MSG_6));
	else if (element_type == TYPE_TEXTURE)
	{
		if (is_texture_valid(fd_sd, parsing, first_letter, parsing->buff[0]))
			return (RETURN_FAILURE);
	}
	else if (element_type == TYPE_RGB)
	{
		if (parsing->buff[0] != ' ' && ft_isnum((int)parsing->buff[0]))
			return (error_handler(parsing, INV_CON,
					"parsing_collect_elements.c:34 ", MSG_7));
		else if (is_rgb_valid(fd_sd, parsing, first_letter, false))
			return (RETURN_FAILURE);
	}
...
static bool	browse_to_find_elements(int fd_sd, t_parsing *parsing, char *read_buff)
{
	int	element_type;
	...
	while (an_element_is_missing(parsing))
	{
		...
		{
			if (element_found(fd_sd, parsing, read_buff[0], element_type))
				return (RETURN_FAILURE);
		}
...
bool	prepare_collect_elements(char *cub_file, t_parsing *parsing, int *fd_sd,
		char **read_buff)
{
	*fd_sd = open_sd_file_give_fd(cub_file);
	if (*fd_sd == OPEN_FAILED)
		return (error_handler(parsing, INV_FAIL, "parsing_collect_elements.c:126 ",
				MSG_4));
	*read_buff = malloc(2);
	if (!*read_buff)
	{
		close(*fd_sd);
		return (error_handler(parsing, MAL_ERR_BUFF,
				"parsing_collect_elements.c:130 ", MSG_5));
	}
	(*read_buff)[1] = '\0';
	if (browse_to_find_elements(*fd_sd, parsing, *read_buff))
	{
		close(*fd_sd);
		return (RETURN_FAILURE);
	}
