...
static int	reopen_file_and_skip_elements(char *file, t_settings *set, int fd)
{
	char	*path;
	int		new_fd;
	...
	if (!path)
	{
		error_handler(set, MAL_ERR_SET, "map_collect.c.c:45 ", MSG_1);
		return (MALLOC_ERR);
	}
...
bool	map_collect(t_settings *set, char *file, int fd)
{
	...
	if (malloc_map_height(set, map_height))
		return (RETURN_FAILURE);
	fd = reopen_file_and_skip_elements(file, set, fd);
